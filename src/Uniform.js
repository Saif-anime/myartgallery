...
function Uniform() {
    ...
    const fetchData = async (id) => {
        if (id) {
            try {
                const resp = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/Uniform_type/${id}`);
                if (resp.ok) {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/Uniform_type`);
                if (response.ok) {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                toast.success('Uniform_type added successfully!');

                // Fetch updated data
                fetchData();
            } else {
                toast.error('Failed to upload Uniform_type. Please try again.');
            }
        } catch (error) {
            console.error('Error uploading Uniform_type', error);
            toast.error('Failed to upload Uniform_type. Please try again.');
        }
    };
    ...
    const BannerShowFun = async (id, isActive) => {
        ...
        if (isActive === 1) {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/Uniform_type/${id}`, {
                    method: 'DELETE',
                    body: isActive,
                });
                ...
                if (response.ok) {
                    ...
                    // Show success toast notification
                    toast.error('Uniform_type Hide !');

                    // Fetch updated data
                    fetchData();
                } else {
                    toast.error('Failed to Uniform_typeshow. Please try again.');
                }
            } catch (error) {
                console.error('Error uploading Uniform_type:', error);
                toast.error('Failed to upload Uniform_type. Please try again.');
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    toast.error('Failed to Uniform_type show. Please try again.');
                }
            } catch (error) {
                console.error('Error uploading Uniform_type:', error);
                toast.error('Failed to upload Uniform_type. Please try again.');
    ...
    //             // Show success toast notification
    //             toast.success('Uniform_type update successfully!');
    //         } else {
    //             // Show success toast notification
    //             toast.success('failed to update Uniform_type successfully!');
    //         }
    //     } catch (error) {
    //         console.error('Error updating Uniform_type:', error);
    //     }
    // }
        ...
        <>
                    ...
                    <div class="modal-content">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title">Uniform Add</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">

                                <div class="row mb-3">
                                    <label for="inputText" class="col-sm-12 col-form-label">Uniform Name</label>
                                    <div class="col-sm-12">
            ...
            <div class="modal fade" id="ExtralargeModal2" tabindex="-1">
                <div class="modal-dialog ">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Uniform Update</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                                ...
                                <div class="col-12">
                                    <div class="card top-selling overflow-auto">
                                        ...
                                        <div class="card-body pb-0">
                                            <h5 class="card-title">Uniform Table</h5>
                                                ...
                                                <tbody>
                                                    {
                                                        data.map((v) => {
                                                            return <tr key={v._id}>
                                                                <td scope="row">{v.Uniform_type_name}</td>
