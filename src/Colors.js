...
function Colors() {
    ...
    const fetchData = async (id) => {
        if (id) {
            try {
                const resp = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/color/${id}`);
                if (resp.ok) {
            ...
            try {
                const response = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/color`);
                if (response.ok) {
    ...
    const BannerShowFun = async (id, isActive) => {
        ...
        if (isActive === 1) {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_FETCH_URL}/Admin/color/${id}`, {
                    method: 'DELETE',
                    body: isActive,
                });
                ...
                if (response.ok) {
                    ...
                    // Show success toast notification
                    toast.error('Color Hide !');

                    // Fetch updated data
                    fetchData();
                } else {
                    toast.error('Failed to Color show. Please try again.');
                }
            } catch (error) {
                console.error('Error uploading Color:', error);
                toast.error('Failed to upload Color. Please try again.');
        ...
        <>
                    ...
                    <div class="modal-content">
                        <form onSubmit={handleSubmit}>
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title">Color Add</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">

                                <div class="row mb-3">
                                    <label for="inputText" class="col-sm-12 col-form-label">Color Name</label>
                                    <div class="col-sm-12">
            ...
            <div class="modal fade" id="ExtralargeModal2" tabindex="-1">
                <div class="modal-dialog ">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">Color Update</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
